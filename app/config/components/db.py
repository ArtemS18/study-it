...
class DatabaseConfig(BaseSettings):
    ...
    def postgres_connection_string(self):
        return (
            self.postgres_dsn
            or f"postgres://{self.postgres_user}:{self.postgres_password}@{self.postgres_host}:{self.postgres_port}/{self.postgres_db}"
        )

    @computed_field(return_type=dict)
    def tortoise_config(self):
        return {
            "connections": {"default": self.postgres_connection_string},
            "apps": {
                "server": {
                    "models": [
                        "aerich.models",
